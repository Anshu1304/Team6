...
REPORT_FILE = 'sqlmediumoutput.txt'
DVWA_PATH = r'C:\xampp\htdocs\dvwa\vulnerabilities\sqli\source\medium.php'  # Update this to your actual DVWA installation path
...
def perform_sql_injection(session, session_id, payload):
    headers = {'Cookie': f'PHPSESSID={session_id}; security=medium'}
...
def extract_vulnerable_sql_code(file_path):
    ...
    with open(file_path, 'r') as file:
        ...
        for i, line in enumerate(lines, start=1):
            if "SELECT" in line and "FROM users" in line and "WHERE user_id =" in line:
...
def run_sql_injection_tests():
    ...
    # SQL Injection payloads for medium level
    payloads = [
        "1 OR 1=1",
